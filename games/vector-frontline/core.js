...
function radarPosition(target,player,scale=8){const dx=target.x-player.x,dz=target.z-player.z,s=Math.sin(player.a),c=Math.cos(player.a);return{x:(dx*c-dz*s)/scale,y:-(dx*s+dz*c)/scale}}
function turnToward(current,target,maxTurn){const d=angleDelta(current,target);return wrapAngle(current+Math.max(-maxTurn,Math.min(maxTurn,d)))}
...
function blocksShot(o,shellHeight=6){return o.type!=='block'||o.h>=shellHeight+4}
...
return{wrapAngle,angleDelta,distance,rng,treadMotion,radarPosition,turnToward,circleBox,blocked,segmentBox,blocksShot,project};});
