(() => {
  ...
  const EMPTY=0,SAND=1,WATER=2,SOIL=3,SEED=4,WOOD=5,FIRE=6,OIL=7,LAVA=8,STONE=9,ICE=10,STEAM=11,SMOKE=12,ACID=13,SALT=14,GLASS=15,METAL=16,PLANT=17,BUG=18,SPARK=19,TNT=20,POWDER=21,CLONE=22,VOID=23,LIGHTNING=24,SLIME=25,RAINBOW=26,SNOW=27,MUD=28,WAX=29,FUSE=30,BOMB=31,MERCURY=32,FOAM=33,OBSIDIAN=34,CLOUD=35,VIRUS=36,ANTIMATTER=37,CONCRETE=38,QUARTZITE=39,BASALT=40,SULFUR=41,COPPER_ORE=42,CRYSTAL=43,GRASS=44;
  const mats = [
  ...
  let mineralTime=180, ambientTemp=20, brushShape='circle', brushSolid=true, drawTool='brush', rulerMode=false, shapeStart=null;
  ...
  function openElementMenu(e){const v=nearestElement(pointerCell(e));contextElement=v;document.querySelector('#elementMenuTitle').textContent=`${mats[v][0]} controls`;document.querySelector('#elementReactions').checked=!!reactionEnabled[v];document.querySelector('#elementMovement').checked=!!movementEnabled[v];document.querySelector('#elementMenu').showModal()}
  canvas.addEventListener('pointerdown',e=>{if(e.button===2){e.preventDefault();e.stopPropagation();openElementMenu(e);return}if(e.pointerType==='touch'){const p=pointerCell(e),now=Date.now(),same=lastTapCell&&Math.abs(p.x-lastTapCell.x)<=brush&&Math.abs(p.y-lastTapCell.y)<=brush;if(same&&now-lastTapAt<420){e.preventDefault();openElementMenu(e);lastTapAt=0;return}lastTapAt=now;lastTapCell=p}if(inspectMode){inspect(e);return}drawing=true;lastPaint=null;shapeStart=pointerCell(e);canvas.setPointerCapture(e.pointerId);if(drawTool==='brush'&&!rulerMode)paint(e)});canvas.addEventListener('pointermove',e=>{inspect(e);if(drawing&&drawTool==='brush'&&!rulerMode)paint(e)});canvas.addEventListener('pointerleave',()=>document.querySelector('#inspector').hidden=true);canvas.addEventListener('pointerup',e=>{if(drawing&&(drawTool!=='brush'||rulerMode))drawShape(shapeStart,pointerCell(e));drawing=false;lastPaint=null;shapeStart=null});canvas.addEventListener('pointercancel',()=>{drawing=false;lastPaint=null;shapeStart=null});window.addEventListener('contextmenu',e=>{e.preventDefault();e.stopPropagation()},true);
  ...
  document.querySelector('#drawTool').onchange=e=>{drawTool=e.target.value;rulerMode=false;document.querySelector('#ruler').setAttribute('aria-pressed','false')};
  document.querySelector('#ruler').onclick=e=>{rulerMode=!rulerMode;if(rulerMode)drawTool='line';e.currentTarget.setAttribute('aria-pressed',String(rulerMode));e.currentTarget.textContent=rulerMode?'📏 Ruler on':'📏 Ruler';document.querySelector('#drawTool').value=drawTool};
