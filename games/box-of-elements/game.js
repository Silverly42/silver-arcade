(() => {
  ...
  const EMPTY=0,SAND=1,WATER=2,SOIL=3,SEED=4,WOOD=5,FIRE=6,OIL=7,LAVA=8,STONE=9,ICE=10,STEAM=11,SMOKE=12,ACID=13,SALT=14,GLASS=15,METAL=16,PLANT=17,BUG=18,SPARK=19,TNT=20,POWDER=21,CLONE=22,VOID=23,LIGHTNING=24,SLIME=25,RAINBOW=26,SNOW=27,MUD=28,WAX=29,FUSE=30,BOMB=31,MERCURY=32,FOAM=33,OBSIDIAN=34,CLOUD=35,VIRUS=36,ANTIMATTER=37,CONCRETE=38,QUARTZITE=39,BASALT=40,SULFUR=41,COPPER_ORE=42,CRYSTAL=43,GRASS=44,ANTIVIRUS=45,NUKE_POWDER=46;
  const mats = [
    ...
    ['Quartzite','#d8cbd0'],['Basalt','#34383d'],['Sulfur','#e7d83a'],['Copper Ore','#8c5b43'],['Crystal','#9de8ee'],['Grass','#55c94f'],
  ...
  function growReactionPatch(x,y,product,sources,max=30,delay=12){
    if(life[idx(x,y)]<18||life[idx(x,y)]%delay!==0)return;
  ...
  function updateCell(x,y){const i=idx(x,y),v=grid[i];if(!v)return;life[i]++;
    ...
    if(v===WATER){if(near(x,y,LAVA)){grid[i]=STEAM;const p=near(x,y,LAVA);if(p)set(p[0],p[1],OBSIDIAN);return}const sulfur=near(x,y,SULFUR);if(sulfur&&life[i]>45&&chance(.025)){set(sulfur[0],sulfur[1],ACID);grid[i]=STEAM;return}if((near(x,y,FIRE)||ambientTemp>=100)&&life[i]>18&&chance(ambientTemp>=100?.12:1)){grid[i]=STEAM;life[i]=0;return}const salt=near(x,y,SALT);if(salt&&chance(.12)){set(salt[0],salt[1],EMPTY);return}const dirt=near(x,y,SOIL);if(dirt&&chance(.015)){set(dirt[0],dirt[1],MUD);if(chance(.35))grid[i]=MUD;return}if(ambientTemp<0&&life[i]>30&&chance(.0008+Math.abs(ambientTemp)*.00008)){set(x,y,ICE);life[i]=0}else updateLiquid(x,y,4);return}
    if(v===OIL){updateLiquid(x,y,5);return}
    if(v===ACID){const metal=near(x,y,METAL);if(metal&&chance(.045)){set(metal[0],metal[1],SPARK);if(chance(.25))set(x,y,SMOKE);return}for(const t of [WOOD,PLANT,GRASS,SEED,BUG,STONE]){const p=near(x,y,t);if(p&&chance(.08)){set(p[0],p[1],EMPTY);if(chance(.15))set(x,y,EMPTY);return}}updateLiquid(x,y,3);return}
    if(v===LAVA){burnAround(x,y);const p=near(x,y,WATER);if(p){set(p[0],p[1],STEAM);set(x,y,OBSIDIAN);return}const metal=near(x,y,METAL);if(metal&&chance(.018)){set(metal[0],metal[1],SPARK);return}if(life[i]>900+ambientTemp*4&&chance(.03)){grid[i]=chance(.82)?BASALT:(chance(.55)?SULFUR:COPPER_ORE);life[i]=0;return}updateLiquid(x,y,1);return}
    ...
    if(v===SEED){if(y+1<H&&grid[idx(x,y+1)]===SOIL&&life[i]>18){grid[i]=GRASS;life[i]=0}else if(y+1<H&&grid[idx(x,y+1)]===SAND&&near(x,y,WATER)&&life[i]>25){grid[i]=PLANT;life[i]=0}else updatePowder(x,y);return}
    if(v===GRASS){if(hot(x,y)){grid[i]=FIRE;life[i]=0;return}if(y+1<H&&grid[idx(x,y+1)]===SOIL&&life[i]%45===0){for(const dx of chance(.5)?[-1,1]:[1,-1])if(empty(x+dx,y)&&inside(x+dx,y+1)&&grid[idx(x+dx,y+1)]===SOIL){set(x+dx,y,GRASS);break}}const water=near(x,y,WATER);if(water&&life[i]>80&&chance(.003))set(x,y,PLANT,22);return}
    ...
    if(v===NUKE_POWDER){if(near(x,y,FIRE)||near(x,y,SPARK)||near(x,y,LAVA)||near(x,y,LIGHTNING)||life[i]>1){if(life[i]>35)explode(x,y,24)}else{life[i]=0;updatePowder(x,y)}return}
  ...
  function paint(e){const p=pointerCell(e),v=selected;if(selected===LIGHTNING){strikeLightning(p.x,p.y);lastPaint=p;return}const from=lastPaint||p,steps=Math.max(1,Math.ceil(Math.hypot(p.x-from.x,p.y-from.y)/(Math.max(1,brush*.45))));for(let s=0;s<=steps;s++)stamp(Math.round(from.x+(p.x-from.x)*s/steps),Math.round(from.y+(p.y-from.y)*s/steps),v);lastPaint=p}
  function drawShape(a,b){const v=selected,dx=b.x-a.x,dy=b.y-a.y;if(drawTool==='line'){const n=Math.max(1,Math.ceil(Math.hypot(dx,dy)));for(let s=0;s<=n;s++)stamp(Math.round(a.x+dx*s/n),Math.round(a.y+dy*s/n),v)}else if(drawTool==='rectangle'){for(let x=Math.min(a.x,b.x);x<=Math.max(a.x,b.x);x++){stamp(x,a.y,v);stamp(x,b.y,v)}for(let y=Math.min(a.y,b.y);y<=Math.max(a.y,b.y);y++){stamp(a.x,y,v);stamp(b.x,y,v)}}else{const rx=Math.abs(dx),ry=Math.abs(dy),n=Math.max(24,Math.ceil(Math.PI*Math.max(rx,ry)*2));for(let s=0;s<n;s++){const q=s*Math.PI*2/n;stamp(Math.round(a.x+Math.cos(q)*rx),Math.round(a.y+Math.sin(q)*ry),v)}}}
  function nearestElement(p){if(inside(p.x,p.y)&&grid[idx(p.x,p.y)]!==EMPTY)return grid[idx(p.x,p.y)];for(let r=1;r<=Math.max(3,brush);r++)for(let yy=-r;yy<=r;yy++)for(let xx=-r;xx<=r;xx++)if(Math.abs(xx)===r||Math.abs(yy)===r){const x=p.x+xx,y=p.y+yy;if(inside(x,y)&&grid[idx(x,y)]!==EMPTY)return grid[idx(x,y)]}return selected}
  function openElementMenuFor(v){contextElement=v;document.querySelector('#elementMenuTitle').textContent=`${mats[v][0]} controls`;document.querySelector('#elementReactions').checked=!!reactionEnabled[v];document.querySelector('#elementMovement').checked=!!movementEnabled[v];document.querySelector('#elementMenu').showModal()}
  function openElementMenu(e){openElementMenuFor(nearestElement(pointerCell(e)))}
  canvas.addEventListener('pointerdown',e=>{if(e.button===2){e.preventDefault();e.stopPropagation();openElementMenu(e);return}if(inspectMode){inspect(e);return}drawing=true;lastPaint=null;shapeStart=pointerCell(e);previewEnd=shapeStart;canvas.setPointerCapture(e.pointerId);if(drawTool==='brush')paint(e)});canvas.addEventListener('pointermove',e=>{inspect(e);if(drawing){previewEnd=pointerCell(e);if(drawTool==='brush')paint(e)}});canvas.addEventListener('pointerleave',()=>document.querySelector('#inspector').hidden=true);canvas.addEventListener('pointerup',e=>{if(drawing&&drawTool!=='brush')drawShape(shapeStart,pointerCell(e));drawing=false;lastPaint=null;shapeStart=null;previewEnd=null});canvas.addEventListener('pointercancel',()=>{drawing=false;lastPaint=null;shapeStart=null;previewEnd=null});window.addEventListener('contextmenu',e=>{e.preventDefault();e.stopPropagation()},true);
  const panel=document.querySelector('#materials');mats.forEach((m,i)=>{if(i===CLONE)return;const b=document.createElement('button');b.className='material'+(i===selected?' active':'')+(hiddenElements.has(i)?' hidden-element':'');b.dataset.element=i;b.innerHTML=`<i style="background:${m[1]}"></i>${m[0]}`;b.onclick=()=>{selected=i;inspectMode=false;document.querySelector('#pinpointer').setAttribute('aria-pressed','false');document.querySelectorAll('.material').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.querySelector('#selectedName').textContent=m[0]};b.addEventListener('pointerup',e=>{if(e.pointerType!=='touch')return;const now=Date.now();if(lastMaterialTap===i&&now-lastMaterialTapAt<420){e.preventDefault();openElementMenuFor(i);lastMaterialTap=-1;lastMaterialTapAt=0}else{lastMaterialTap=i;lastMaterialTapAt=now}});panel.appendChild(b)});
