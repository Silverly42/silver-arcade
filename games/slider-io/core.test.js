...
test('death-body sampling caps pellet count while covering the body',()=>{const points=Array.from({length:700},(_,i)=>({x:i,y:0})),sample=C.sampleBody(points,120);assert.equal(sample.length,120);assert.equal(sample[0].x,0);assert.ok(sample.at(-1).x>690)});
test('food hoover pulls nearby food and ignores distant food',()=>{const player={x:100,y:0};assert.equal(C.foodPull({x:-300,y:0},player,280).strength,0);const pull=C.foodPull({x:0,y:0},player,280);assert.ok(pull.x>0&&pull.strength>0)});
