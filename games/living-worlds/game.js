(() => {
  ...
  const EMPTY=0,SAND=1,WATER=2,SOIL=3,SEED=4,WOOD=5,FIRE=6,OIL=7,LAVA=8,STONE=9,ICE=10,STEAM=11,SMOKE=12,ACID=13,SALT=14,GLASS=15,METAL=16,PLANT=17,BUG=18,SPARK=19;
  const mats = [
  ...
  function burnAround(x,y){for(let yy=-1;yy<=1;yy++)for(let xx=-1;xx<=1;xx++){if(!inside(x+xx,y+yy))continue;const i=idx(x+xx,y+yy),v=grid[i];if((v===WOOD||v===PLANT||v===SEED||v===OIL)&&chance(v===OIL?.3:.025)){grid[i]=FIRE;life[i]=0}}}
  function updateCell(x,y){const i=idx(x,y),v=grid[i];if(!v)return;life[i]++;
    if(v===SAND||v===SALT||v===SOIL){updatePowder(x,y);return}
  ...
  function render(){const img=ctx.createImageData(W,H),d=img.data;count=0;for(let i=0;i<N;i++){const v=grid[i];if(v)count++;let hex=color[v],n=parseInt(hex.slice(1),16);let r=n>>16,g=n>>8&255,b=n&255;const noise=((i*13+tick*7)%9)-4;if(v!==EMPTY){r+=noise;g+=noise;b+=noise}d[i*4]=r;d[i*4+1]=g;d[i*4+2]=b;d[i*4+3]=255}ctx.putImageData(img,0,0);document.querySelector('#stats').textContent=`${count.toLocaleString()} particles`}
  ...
  document.querySelector('#save').onclick=()=>{localStorage.setItem('living-worlds-save',JSON.stringify(Array.from(grid)));document.querySelector('#save').textContent='Saved!';setTimeout(()=>document.querySelector('#save').textContent='Save',900)};
  document.querySelector('#load').onclick=()=>{try{const a=JSON.parse(localStorage.getItem('living-worlds-save'));if(a&&a.length===N)grid.set(a)}catch{alert('That save could not be loaded.')}};
